...
from saas_insights.tco import calculate_tco


def test_tco_has_traceable_components() -> None:
    row = {
        "modernization_asset_count": 500,
        "primary_annual_contract_value_jpy_mn": 200,
        "primary_platform_share_pct": 85,
    ...
    assert len(result) == 5
    assert result["primary_jpy_mn"].sum() > 0
